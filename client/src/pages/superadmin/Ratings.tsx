...
import { Star, TrendingUp, Users, Info, Edit } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
                    ...
                    <TableRow key={supplier.id}>
                      ...
                      <TableCell>
                        {supplier.isVerified ? (
                          ...
                          <Badge variant="outline">{supplier.verificationStatus || "Pending"}</Badge>
                        )}
                      </TableCell>
                      <TableCell>
              ...
              {suppliersData && suppliersData.totalPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <div className="text-sm text-muted-foreground">
                    Page {suppliersData.page} of {suppliersData.totalPages} ({suppliersData.totalCount} total)
                  </div>
      ...
      <Dialog open={selectedSupplier !== null} onOpenChange={(open) => !open && setSelectedSupplier(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Adjust Supplier Rating</DialogTitle>
            <DialogDescription>
              Manually adjust the supplier's rating. This will affect their ranking in job matching.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="rating">New Rating (1.0 - 5.0)</Label>
              <div className="flex items-center gap-4">
                <Input
                  ...
                  step="0.1"
                  value={newRating / 100}
                  onChange={(e) => setNewRating(Math.round(parseFloat(e.target.value) * 100))}
                  className="w-32"
                />
                <Badge variant={getRatingBadgeVariant(newRating)}>
                  <Star className="h-3 w-3 mr-1" />
                  {formatRating(newRating)}
                </Badge>
              </div>
            </div>
